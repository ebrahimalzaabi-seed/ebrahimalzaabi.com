...
const PORT = 3111;
const PROJECT_ROOT = lib.PROJECT_ROOT;
...
async function handleRequest(req, res) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/fatawa') {
    ...
    res.end(JSON.stringify(lib.getFatwaList()));
    return;
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/youtube-transcript') {
    ...
    const data = await lib.fetchTranscript(ytUrl);
  ...
  if (req.method === 'GET' && url.pathname === '/api/fatwa-meta') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify(lib.getFatwaMeta()));
    return;
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/create-fatwa') {
    const body = await parseBody(req);
    const result = lib.createFatwa(body);
  ...
  if (req.method === 'GET' && url.pathname.startsWith('/api/fatwa/')) {
    ...
    const fatwa = lib.getFatwa(fatwaId);
    if (!fatwa) {
  ...
  if (req.method === 'PUT' && url.pathname.startsWith('/api/fatwa/')) {
    const fatwaId = url.pathname.replace('/api/fatwa/', '');
    const body = await parseBody(req);
    const result = lib.updateFatwa(fatwaId, body);
    const status = result.output === 'Fatwa not found' ? 404 : (result.success ? 200 : 400);
  ...
  if (req.method === 'DELETE' && url.pathname.startsWith('/api/fatwa/')) {
    const fatwaId = url.pathname.replace('/api/fatwa/', '');
    const result = lib.deleteFatwa(fatwaId);
    const status = result.output === 'Fatwa not found' ? 404 : (result.success ? 200 : 500);
    res.writeHead(status, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify(result));
    return;
  }
